...
<body>
		...
		<table border="1">
			...
			<tbody>
				<c:forEach var="file" items="${fileList }">
					<tr>
						<td><label><input type="checkbox" name="fileIds"
								value="${file.fileId }"> ${file.fileId }</label></td>
						<td>${file.directoryName }</td>
						<td><c:url var="contentUrl" value="/file/${file.fileId }" />
							<%-- 확장자 --%> 
							<c:set var="len" value="${fn:length(file.fileName) }" />
								<c:set var="filetype" value="${fn:toUpperCase(fn:substring(file.fileName, len-4, len)) }" />
							<c:choose>
								<c:when
									test="${(filetype eq '.JPG') or (filetype eq '.GIF') or (filetype eq '.PNG')}">
									<img src="${contentUrl }" style="width: 100px">
								</c:when>
								<c:otherwise>
						...
						<td><fmt:formatDate value="${file.fileUploadDate }"
								pattern="yyyy-MM-dd" /></td>
						<td><c:url var="deleteUrl"
								value="/upload/delete/${file.fileId }" /> <a
							href="${deleteUrl }" class="delete">삭제</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<p>
		<a href='<c:url value="/upload/new"/>'>업로드</a> <a
			href='<c:url value="/upload"/>'>처음으로</a>
	</p>
	<script>
		// 요소선택자
		...
		$('#form01').on('submit', function(e) {
			e.preventDefault();
			let cnt = $('input[name="fileIds"]:checked').length;
			console.log(cnt);
			if (cnt>0){
