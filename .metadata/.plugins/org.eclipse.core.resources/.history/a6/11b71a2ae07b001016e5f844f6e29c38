...
public class CalcAPIServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int num1 = Optional.ofNullable(request.getParameter("num1")).map(Integer::parseInt).orElse(0);
		int num2 = Optional.ofNullable(request.getParameter("num2")).map(Integer::parseInt).orElse(0);
